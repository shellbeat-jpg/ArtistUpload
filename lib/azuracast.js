...
async function importExistingFile(stationIdOrShortcode, relativePath) {
    const baseUrl = process.env.AZURACAST_BASE_URL;
    const apiKey = process.env.AZURACAST_API_KEY;

    // Weist AzuraCast an, eine bereits im Volume liegende Datei einzulesen
    const url = `${baseUrl}/api/station/${stationIdOrShortcode}/media`;
    ...
    const response = await fetch(url, {
        ...
        body: JSON.stringify({
            path: relativePath // z.B. "incoming/abc.wav" oder "mapped-to-playlist/abc.wav"
        })
    });
...
// Stelle sicher, dass importExistingFile unten in module.exports exportiert wird!
