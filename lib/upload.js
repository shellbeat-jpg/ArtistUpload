...
function fileFilter(req, file, cb) {
    ...
    if (file.fieldname === 'image') {
        if (!allowedImageExtensions.includes(ext)) {
            return cb(new Error(`Bildformat .${ext} ist nicht erlaubt. Erlaubt: ${allowedImageExtensions.join(', ')}`));
        }
        return cb(null, true);
    }

    if (!allowedAudioExtensions.includes(ext)) {
        return cb(new Error(`Dateityp .${ext} ist nicht erlaubt. Erlaubt: ${allowedAudioExtensions.join(', ')}`));
    }
    cb(null, true);
}
...
function validateImageSize(file) {
    if (file && file.size > maxImageSizeBytes) {
        throw new Error(`Bild ist zu gross (max. ${maxImageSizeBytes / (1024 * 1024)} MB).`);
    }
}
